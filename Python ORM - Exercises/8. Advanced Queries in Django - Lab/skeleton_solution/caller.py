...
def product_quantity_ordered():
    products = (Product.objects.annotate(
        product_quantity=Sum('orderproduct__quantity'))
                .exclude(product_quantity=None)
                .order_by('-product_quantity')
        )
    result = []
    for product in products:
        result.append(f"Quantity ordered of {product.name}: {product.product_quantity}")
...
def filter_products():
    filtered_products = Product.objects.filter(Q(price__gt=3) & Q(is_available=True)).order_by('-price', 'name')
    result = []

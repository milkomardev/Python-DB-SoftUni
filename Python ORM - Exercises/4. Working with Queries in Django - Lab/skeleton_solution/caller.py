...
def find_authors_nationalities():
    ...
    return '\n'.join(f'{str(x)} is {x.nationality}' for x in authors)


def order_books_by_year():
    books = Book.objects.all().order_by('publication_year', 'title')

    return '\n'.join(f'{b.publication_year} year: {str(b)}' for b in books)
...
def filter_authors_by_nationalities(nationality):
    ...
    return '\n'.join(
        f'{a.biography}' if a.biography is not None
        else str(a)
        for a in authors
    )
...
def filter_authors_by_birth_year(first_year, second_year):
    ...
    return '\n'.join(
        f"{a.birth_date}: {str(a)}" for a in authors)


def change_reviewer_name(reviewer_name, new_reviewer_name):
